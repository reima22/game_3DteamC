...
void SetEffect(D3DXVECTOR3 pos, D3DXVECTOR3 move ,int nLife, int nType, float fWidth, float fHeigth)
{
	...
	for (int nCntEffect = 0; nCntEffect < MAX_EFFECT; nCntEffect++, pEffect++)
	{
		if (g_Effect[nCntEffect].bUse == false)
		{
			...
			////頂点座標
			//pVtx[0].pos = D3DXVECTOR3(pEffect->pos.x - pEffect->fHeigth, pEffect->pos.y + pEffect->fHeigth, 0.0f);
			//pVtx[1].pos = D3DXVECTOR3(pEffect->pos.x + pEffect->fHeigth, pEffect->pos.y + pEffect->fHeigth, 0.0f);
			//pVtx[2].pos = D3DXVECTOR3(pEffect->pos.x - pEffect->fHeigth, pEffect->pos.y - pEffect->fHeigth, 0.0f);
			//pVtx[3].pos = D3DXVECTOR3(pEffect->pos.x + pEffect->fHeigth, pEffect->pos.y - pEffect->fHeigth, 0.0f);
