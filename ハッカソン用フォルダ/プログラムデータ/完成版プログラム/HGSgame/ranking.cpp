...
void UpdateRanking(void)
{
	...
	VERTEX_2D *pVtx;
	FADE fade;
	TITLEAUTO titleauto = GetTitle();	// 自動遷移情報の取得
	int nScore = GetScore();	// スコアの取得
	...
	// タイトルへの切り替え
	if (GetKeyboardTrigger(KEYINFO_OK) == true || IsButtonDown(KEYINFO::KEYINFO_SHOT) == true && fade == FADE_NONE || g_nCounter >= 300)
	{
		SetFade(FADE_OUT, MODE_TITLE);
	}

	if (titleauto == TITLE_AUTO)
	{
		g_nCounter++;
	}
}
