...
void UpdateBlock(void)
{
	...
	{// 30秒まで
		...
		if (g_nSetBlock == 0 || g_nSetBlock % 250 == 0)
		{
			for (int nCnt = 0; nCnt < BLOCK_SET; nCnt++)
			{
				if (nCnt == nBreak)
	...
	else if (nTimer <= 90 && nTimer > 60)
	{// 60秒
		if (g_nSetBlock % 200 == 0)
		{
			for (int nCnt = 0; nCnt < BLOCK_SET; nCnt++)
			{
				if (nCnt == nBreak)
	...
	else if (nTimer <= 60 && nTimer > 30)
	{// 90秒まで
		if (g_nSetBlock % 150 == 0)
		{
			for (int nCnt = 0; nCnt < BLOCK_SET; nCnt++)
			{
				if (nCnt == nBreak)
