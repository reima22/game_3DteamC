...
void DrawFPS(void)
{
	...
	nNum += sprintf(&aStr[nNum], "モーション毎のトータルカウント：[%d / %d]\n", pPlayer->attack.nCntFrameTotal, pPlayer->nFrameTotal[pPlayer->motionType]);
	nNum += sprintf(&aStr[nNum], "キー毎のトータルカウント：[%d / %d]", pPlayer->nCounterMotion, pPlayer->aMotionInfo[pPlayer->motionType].aKeySet[pPlayer->nKey].nFrame);
